...
namespace Tangzx.ABSystem
{
    ...
    public class AssetBundlePathResolver
    {
        ...
        /// AB 保存的路径
        /// </summary>
        public string BundleSavePath { get { return Application.persistentDataPath + "/" + BundleSaveDirName; } }
...
#if UNITY_EDITOR
            if (forWWW)
                filePath = string.Format("file://{0}/{1}/{2}", Application.persistentDataPath, BundleSaveDirName, path);
            else
                filePath = string.Format("{0}/{1}/{2}", Application.persistentDataPath, BundleSaveDirName, path);
#elif UNITY_ANDROID
            if (forWWW)
                filePath = string.Format("jar:file://{0}!/assets/{1}/{2}", Application.persistentDataPath, BundleSaveDirName, path);
            else
                filePath = string.Format("{0}!assets/{1}/{2}", Application.persistentDataPath, BundleSaveDirName, path);
#elif UNITY_IOS
            if (forWWW)
                filePath = string.Format("file://{0}/Raw/{1}/{2}", Application.persistentDataPath, BundleSaveDirName, path);
            else
                filePath = string.Format("{0}/Raw/{1}/{2}", Application.persistentDataPath, BundleSaveDirName, path);
#else
        ...
        /// AB 依赖信息文件名
        /// </summary>
        public virtual string DependFileName { get { return "dep.txt"; } }
